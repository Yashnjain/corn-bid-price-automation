...
# receiver_email = 'manish.gupta@biourja.com,devina.ligga@biourja.com,Adarsh.Bhandari@biourja.com,radha.waswani@biourja.com'
receiver_email = 'indiapowerit@biourja.com'
# receiver_email = 'manish.gupta@biourja.com,radha.waswani@biourja.com'
#path = 'Cornbids.xlsx'
path=r"S:\IT Dev\Production_Environment\cron-bid-price-automation"

logfile = os.getcwd() + '\\' + str(datetime.today().date()) + '_CORNBIDPRICE_LOG_ALERT.txt'
...
def initialize_new_sheet(bid_prices):
    try:
        ...
        print("Error occoured in initalizing new sheet ",e)
...
def scrape_absenergy():
    ...
    try:
        ...
        print(sys.exc_info()[0])
        print("Error occoured with website: http://www.absenergy.org/grainbids.html:::::::",e)
        logger.info(sys.exc_info()[0])
...
def scrape_fhr(url):
    ...
    try:
        driver.get(url)
        WebDriverWait(driver, 15).until(EC.element_to_be_clickable((By.CLASS_NAME, 'pricingTable')))
        soup = BeautifulSoup(driver.page_source, features='lxml')
        ...
        print(sys.exc_info()[0])
        print("Error occured with website:", url)
        logger.info(sys.exc_info()[0])
        logger.info(f"Error occured with website:{url}")
        return month_to_basis
...
def fetch_and_insert_regular_websitedata():
    ...
    bids = scrape_regular_website_2(url="https://www.hankinsonre.com/janesville", wait_by_option=1, basis_index=3,
                                    ...
                                    row_start_index=2,
                                    xpath_for_table="/html/body/div[1]/div[2]/div[2]/div/div[3]/div[1]/div[2]/div[2]/div[1]/table/tbody/tr/td/div/div/div/table")
    ...
    bids = scrape_regular_website_2(url="https://kaapaethanolcommodities.com/Commodities/Cash-Bids", basis_index=5,
                                    month_index=1, row_start_index=2, table_index=3,find_by_option=1, 
                                    class_name="cashbid_table cashbid_fulltable", row_end_index=8)
    ...
    bids = scrape_regular_website_2(url="https://www.hankinsonre.com/hankinson", wait_by_option=1, basis_index=3,
                                        month_index=1, find_by_option=1, class_name="cashbid_table cashbid_fulltable",
                                        row_start_index=2, row_end_index=8,
                                        xpath_for_table="/html/body/div[1]/div[2]/div[2]/div/div[2]/div[1]/div[2]/div[2]/div[1]/table/tbody/tr/td/div/div/div/table")
    ...
    bids = scrape_regular_website_2(url="https://siouxlandethanol.com/cash-bids/", wait_by_option=1, find_by_option=1,
                                    basis_index=-2, xpath_for_table="/html/body/div[3]/div[1]/div/div/div/table",class_name="cashbid_table cashbid_fulltable",
                                    row_start_index=2)
    ...
    bids = scrape_regular_website_2(url="https://www.hankinsonre.com/lima", wait_by_option=1, basis_index=3,
                                        ...
                                        xpath_for_table="/html/body/div[1]/div[2]/div[2]/div/div[4]/div[1]/div[2]/div[2]/div[1]/table/tbody/tr/td/div/div/div/table")
    ...
    bids = scrape_regular_website_2(url="http://poetbiorefining-mitchell.aghost.net/index.cfm?show=11&mid=3",
                                    month_index=0, basis_index=4, class_name='DataGrid', row_start_index=2, wait_by_option=3,
                                    find_by_option=1)
    if insert_into_sheet(155, bids):
        print("success for row 154")
        logger.info("success for row 154")
...
def insert_into_sheet(row_number, bids):
    if bids:
        try:
            ...
            print(sys.exc_info()[0])
            logger.info(sys.exc_info()[0])
            logger.info(f"Error occoured in insert_into_sheet {e}")
            raise e
            # return False
    else:
...
def main():
    ...
    try:
        ...
        print(sys.exc_info()[0])
        logger.info(f"Error occoured in main {ex}")
        logger.info(sys.exc_info()[0])
        raise e
...
if __name__ == "__main__":
    ...
    try:
        ...
        bu_alerts.bulog(process_name="CORN_BID_PRICE_SCRAPPER", database='POWERDB_DEV',status='Started',table_name = '', row_count=0, log=log_json, warehouse='BUIT_WH',process_owner='Manish')
        main()
        log_json='[{"JOB_ID": "'+str(job_id)+'","CURRENT_DATETIME": "'+str(datetime.datetime.now())+'"}]'
        bu_alerts.bulog(process_name="CORN_BID_PRICE_SCRAPPER", database='POWERDB_DEV',status='Completed',table_name = '', row_count=0, log=log_json, warehouse='BUIT_WH',process_owner='Manish')
        logging.info('Execution Done')
        bu_alerts.send_mail(
            receiver_email = receiver_email,
            mail_subject ='JOB SUCCESS - CORN_BID_PRICE_SCRAPPER',
            mail_body = 'CORN_BID_PRICE_SCRAPPER completed successfully, Attached logs',
        ...
        log_json='[{"JOB_ID": "'+str(job_id)+'","CURRENT_DATETIME": "'+str(datetime.datetime.now())+'"}]'
        bu_alerts.bulog(process_name="CORN_BID_PRICE_SCRAPPER", database='POWERDB_DEV',status='Failed',table_name = '', row_count=0, log=log_json, warehouse='BUIT_WH',process_owner='Manish')
        bu_alerts.send_mail(
            receiver_email = receiver_email,
            mail_subject ='JOB FAILED - CORN_BID_PRICE_SCRAPPER',
            mail_body = 'CORN_BID_PRICE_SCRAPPER failed during execution, Attached logs',
            attachment_location = logfile)
    time_end = time.time()
